...
def shift_scale_points(pred_xyz, src_range, dst_range=None):
    ...
    """
    # Move src_range to same device as pred_xyz
    device = pred_xyz.device
    src_range = [x.to(device) for x in src_range]  # ← ADD THIS
    
    if dst_range is None:
        dst_range = [
            torch.zeros((src_range[0].shape[0], 3), device=device),  # ← Use device
            torch.ones((src_range[0].shape[0], 3), device=device),   # ← Use device
        ]
    else:
        dst_range = [x.to(device) for x in dst_range]  # ← ADD THIS
