...
class TransformerDecoder(nn.Module):
    ...
    def get_proposal_box_predictions_refine(self, idx, query_xyz, point_cloud_dims, box_features, pre_center_normalized=None,pre_size_normalized=None,):
        assert isinstance(point_cloud_dims, (list, tuple)), \
            f"point_cloud_dims should be tuple, got {type(point_cloud_dims)}"
        assert point_cloud_dims[0].ndim == 2 and point_cloud_dims[0].shape[1] == 3, \
            f"point_cloud_dims[0] should be [B, 3], got {point_cloud_dims[0].shape}"
        assert point_cloud_dims[1].ndim == 2 and point_cloud_dims[1].shape[1] == 3, \
            f"point_cloud_dims[1] should be [B, 3], got {point_cloud_dims[1].shape}"
    ...
    def forward(self, tgt, memory, query_xyz, enc_xyz, point_cloud_dims, 
        ...
        for idx, layer in enumerate(self.layers):
            ...
            query_reference = torch.cat([reference_center, reference_size], dim=-1).float()
