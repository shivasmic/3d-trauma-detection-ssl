...
class PositionEmbeddingCoordsSine(nn.Module):
    ...
    def get_fourier_embeddings(self, xyz, num_channels=None, input_range=None):
        ...
        # clone coords so that shift/scale operations do not affect original tensor
        orig_xyz = xyz
        xyz = orig_xyz.clone().float()  # Force float32

        ncoords = xyz.shape[1]
        if self.normalize:
            xyz = shift_scale_points(xyz, src_range=input_range)

        xyz = xyz.float()  # Ensure float32 after shift_scale
        xyz *= 2.0 * math.pi  # Use explicit 2.0 float
        
        # FORCE both tensors to float32
        xyz_proj = torch.mm(
            xyz.view(-1, d_in).float(), 
            self.gauss_B[:, :d_out].float()
        ).view(bsize, npoints, d_out)
