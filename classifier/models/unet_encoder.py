...
class UNet3D(nn.Module):
    ...
    def encode(self, x: torch.Tensor) -> torch.Tensor:
        c1 = self.down1(x)      # [B, 16, 512, 336, 336]
        p1 = self.pool1(c1)     # [B, 16, 256, 168, 168]
        
        c2 = self.down2(p1)     # [B, 32, 256, 168, 168]
        p2 = self.pool2(c2)     # [B, 32, 128, 84, 84]
        
        c3 = self.down3(p2)     # [B, 64, 128, 84, 84]
        p3 = self.pool3(c3)     # [B, 64, 64, 42, 42]
        
        c4 = self.down4(p3)     # [B, 128, 64, 42, 42]
        p4 = self.pool4(c4)     # [B, 128, 32, 21, 21]
        
        bottleneck = self.bottleneck(p4)  # [B, 256, 32, 21, 21]
...
def load_pretrained_unet_encoder(
    pretrained_path: str = 'path to pretrained 3D UNet Encoder Weights',
    freeze_encoder: bool = True,
    device: str = 'cuda'
) -> UNet3D:
    
    print(f"Loading SSL Pre-trained UNet Encoder")
    print(f"Weights file: {pretrained_path}")
